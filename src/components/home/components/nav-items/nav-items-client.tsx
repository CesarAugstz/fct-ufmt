...
export default function NavItemsClient({ sections }: NavItemsClientProps) {
  return (
    <nav className="bg-gradient-to-r from-primary/98 via-primary to-primary/95 text-white py-3 sticky top-0 z-50 shadow-[0_4px_20px_-2px_rgba(0,0,0,0.4)] border-b border-primary/20">
                ...
                <li key={section.name} className="relative group">
                  {section.children && section.children.length > 0 ? (
                    ...
                    <Link href={section.href ?? '#'} className="ml-2">
                      <Button
                        variant="ghost"
                        className="text-white font-medium tracking-wide hover:text-primary-foreground hover:bg-white/10 px-3 py-1.5 h-auto transition-all duration-200 rounded-md text-sm relative after:absolute after:bottom-0 after:left-1/2 after:w-0 after:h-0.5 after:bg-primary-foreground after:transform after:-translate-x-1/2 hover:after:w-2/3 after:transition-all after:duration-300"
...
function NavDropdown({ section }: NavDropdownProps) {
  ...
  if (!hasChildren) {
    return (
      <Button
        variant="ghost"
        className="text-white font-medium tracking-wide hover:text-primary-foreground hover:bg-white/10 px-3 py-1.5 h-auto transition-all duration-200 rounded-md text-sm"
        disabled={!section.href}
      >
        {section.name}
      </Button>
      ...
      <DropdownMenu open={isHovered}>
        <DropdownMenuTrigger asChild>
          <Button
            variant="ghost"
            asChild
            className="text-white font-medium tracking-wide hover:text-primary-foreground hover:bg-white/10 px-3 py-1.5 h-auto flex items-center transition-all duration-200 rounded-md text-sm relative after:absolute after:bottom-0 after:left-1/2 after:w-0 after:h-0.5 after:bg-primary-foreground after:transform after:-translate-x-1/2 group-hover:after:w-2/3 after:transition-all after:duration-300"
        ...
        <DropdownMenuContent
          className="w-64 rounded-lg border border-primary/10 bg-white/98 backdrop-blur-md shadow-[0_10px_40px_-15px_rgba(0,30,80,0.3)] animate-in fade-in-50 zoom-in-95 data-[side=top]:slide-in-from-bottom-2 p-1.5 mt-1"
          sideOffset={8}
